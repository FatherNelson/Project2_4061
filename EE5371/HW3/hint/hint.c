...
main(int argc, char *argv[])
{
    ...
    ISIZE   imax,         /* Maximum representable index                      */
            itm, itm2,    /* Scratch values to find imax                      */
    ...
    double  quips,        /* QUality Improvement Per Second                   */
 ...
 /* This loop is the main loop driver of the HINT kernel.                     */
    ...
    {     
     ...
     /* This is to avoid overflow of dmax                                     */
        quips = (double)dmax / gamut / t + 1.0 / gamut / t;
        qdata[i].t  = t;
        qdata[i].qp = quips;
        qdata[i].n  = n;
    ...
    for (qpnet = 0, j = i - 2; j >= 0; j--)
    {
     /* If more work takes less time, we need to rerun the case of less work. */
        ...
        if (qdata[j+1].t < qdata[j].t)
        {
            ...
            qdata[j].qp = (double)dmax / gamut / qdata[j].t + 
                           1.0 / gamut / qdata[j].t;
        }
