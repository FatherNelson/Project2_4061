...
int list_users(int idx, USER * user_list)
{
  // If the function is called by the server (that is, idx is -1), then printf the list
    ...
    // if every slot is empty, print "<no users>""
		...
		s--;
		strncpy(s, "\n\0", 2); //Place Null character at end of string buffer.
	}
 ...
 */
int broadcast_msg(USER * user_list, char *buf, int idx)
{
  ...
  for (int i = 0; i < MAX_USER; i++) {
    if (user_list[i].m_status == SLOT_EMPTY || idx == i) { // Reached the last user
      continue;
    }
    else {
      write(user_list[i].m_fd_to_user, "\n", 1);
      if (idx != -1) {
        write(user_list[i].m_fd_to_user, user_list[idx].m_user_id, strlen(user_list[idx].m_user_id)); // Cap username length at MAX_MSG
        write(user_list[i].m_fd_to_user, ": ", 2);
      }
      else
        write(user_list[i].m_fd_to_user, "admin: ", 7);
 ...
 */
void send_p2p_msg(int recipient, USER * user_list, char *buf, int sender) {
    // Recipient is checked to be valid before this function is called.
    write(user_list[recipient].m_fd_to_user, "\n", 1);
    write(user_list[recipient].m_fd_to_user, user_list[sender].m_user_id, strlen(user_list[sender].m_user_id)); // Cap username length at MAX_MSG
    write(user_list[recipient].m_fd_to_user, " whispers: ", 11);
    if (strlen(buf) < MAX_CMD_LENGTH)
      write(user_list[recipient].m_fd_to_user, buf, strlen(buf));
    else
    write(user_list[recipient].m_fd_to_user, buf, MAX_CMD_LENGTH);
    write(user_list[recipient].m_fd_to_user, "\n", 1);
}
...
void parse_message(char* cmd_buf, USER user_list[], int idx){
    ...
    switch (found){
  		case LIST:
        /** List **/
  			list_users(idx, user_list); //See definition of list_users, must be called with -1 to make sense.
        break;
   		case KICK:
        /** Kick **/
        if (idx == -1) {
          ...
          broadcast_msg(user_list, cmd_buf, idx);
        }
         break;
   		case P2P:
         /** P2P **/
         if(idx != -1) {
           int recipient = find_user_index(user_list, arg_array[1]);
           if (recipient != -1)
            send_p2p_msg(recipient, user_list, arg_array[2], idx);
           else {
             write(user_list[idx].m_fd_to_user, "User does not exist\n", strlen("User does not exist\n"));
          }
         } else {
           broadcast_msg(user_list, cmd_buf, idx);
         }
...
int main(int argc, char * argv[])
{
    ...
    for (int i = 0; i < MAX_USER; i++) {
      if(user_list[i].m_status != SLOT_EMPTY) {
          ...
          parse_message(rx_buf, user_list, i);
    		}
      }
    }
      ...
      if(find_user_index(user_list, user_id) >= 0){
        write(pipe_client_reading_from_child[1], "User already exists\n", strlen("User already exists\n"));
      } else {
  			printf("\nAdded a new user!\n");
        print_prompt("admin");
